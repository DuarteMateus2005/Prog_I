...
def eFim(partida):
    return len(proximasJogadas(partida))==0

def eValida(partida,jogada):
    dim = partida[0]
    if not isinstance(jogada,tuple) or len(jogada)!=2 or not isinstance(jogada[0],int) or not isinstance(jogada[1],int) or chegoucasafinal(partida):
        return False
...
def vencedor(partida):
    if chegoucasafinal(partida):
        jogadas = partida[1]
        if jogadas[-1][0] == 0:
            return 1
        else:
            return 2
    elif len(partida[1])%2==0:
        return 1
    else:
        return 2
...
def imprimir(partida):
    ...
    for n in range(partida[0],0,-1):
        ...
        for i in range(partida[0]):
            if n == partida[0] and i == partida[0]-1 and t[i][i] == 0:
                res += '2'
